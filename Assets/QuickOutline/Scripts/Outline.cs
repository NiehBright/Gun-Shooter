...
public class Outline : MonoBehaviour {
  ...
  [SerializeField]
  private Color outlineColor = new Color(0.9f, 0.92f, 0.95f, 1f);

  [SerializeField, Range(0f, 10f)]
  private float outlineWidth = 1.9f;
  ...
  void Awake() {
    ...
    // Cache renderers (only MeshRenderer and SkinnedMeshRenderer, avoid ParticleSystemRenderer)
    renderers = GetComponentsInChildren<Renderer>(true).Where(r => r is MeshRenderer || r is SkinnedMeshRenderer).ToArray();
  ...
  void OnDisable() {
    if (renderers == null || outlineMaskMaterial == null || outlineFillMaterial == null) return;
