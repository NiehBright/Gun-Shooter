        ...
        static void SelectItem(Item item)
        {
            void openFolder(EditorWindow browser, string path)
            {
                ...
                if (browser.GetFieldValue<int>("m_ViewMode") == 1)
                {
                    ...
                    // Lấy EntityId của folderAsset (chỉ có trên Unity 6)
                    object folderEntityId = null;
                    try
                    {
                        var getEntityIdMethod = typeof(Object).GetMethod("GetEntityId");
                        if (getEntityIdMethod != null)
                        {
                            folderEntityId = getEntityIdMethod.Invoke(folderAsset, null);
                        }
                    }
                    catch {}
                    
                    // 1. Thử gọi ShowFolderContents (Rất ổn định trên cả Unity cũ và Unity 6)
                    try
                    {
                        var showFolderMethod = browser.GetType().GetMethod("ShowFolderContents", 
                        ...
                        {
                            var firstParamType = showFolderMethod.GetParameters()[0].ParameterType;
                            if (firstParamType.Name == "EntityId" && folderEntityId != null)
                            {
                                showFolderMethod.Invoke(browser, new object[] { folderEntityId, false });
                                success = true;
                            }
                            else if (firstParamType == typeof(int))
                            {
                                showFolderMethod.Invoke(browser, new object[] { folderAsset.GetInstanceID(), false });
                                success = true;
                            }
                        }
                    }
                    ...
                    // 2. Thử gọi SetFolderSelection
                    if (!success)
                    {
                        try
                        {
                            var methods = browser.GetType().GetMethods(
                                System.Reflection.BindingFlags.Instance | 
                                System.Reflection.BindingFlags.Public | 
                                System.Reflection.BindingFlags.NonPublic);
                            
                            System.Reflection.MethodInfo setFolderMethod = null;
                            foreach (var m in methods)
                            {
                                if (m.Name == "SetFolderSelection")
                                {
                                    ...
                                    if (parameters.Length > 0)
                                    {
                                        ...
                                        if (pType.IsArray)
                                        {
                                            ...
                                            if (elementType.Name == "EntityId" && folderEntityId != null)
                                            {
                                                setFolderMethod = m;
                                                break;
                                            }
                            ...
                            if (setFolderMethod != null)
                            {
                                var pType = setFolderMethod.GetParameters()[0].ParameterType;
                                var elementType = pType.GetElementType();
                                
                                object folderArray = null;
                                if (elementType.Name == "EntityId" && folderEntityId != null)
                                {
                                    var arr = System.Array.CreateInstance(elementType, 1);
                                    arr.SetValue(folderEntityId, 0);
                                    folderArray = arr;
                                }
                                else
                                {
                                    folderArray = new[] { folderAsset.GetInstanceID() };
                                }

                                var paramCount = setFolderMethod.GetParameters().Length;
                                if (paramCount == 3)
                                {
                                    setFolderMethod.Invoke(browser, new object[] { folderArray, false, false });
                                }
                                else
                                {
                                    setFolderMethod.Invoke(browser, new object[] { folderArray, false });
                                }
                                success = true;
                            }
                        }
                    ...
                    // 3. Fallback dùng Selection API chính thức
                    if (!success)
                    {
